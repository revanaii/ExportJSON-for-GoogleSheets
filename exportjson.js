...
var HEADER_SKIP_TAG = "HEADER_SKIP_TAG";
...
function exportAllSheets(e) {
  ...
  for (var i = 0; i < sheets.length; i++) {
    ...
    if(sheetName.indexOf("TEST_") > -1 || sheetName == "empty") continue;
...
function getRowsData_(sheet, options) {
  ...
  var allHeaders = headersRange.getValues()[0];
  var headers = [];
  for(var i = 0; i < allHeaders.length; i++) {
    var header = allHeaders[i];
    if(header.indexOf("TEST_") == -1) headers.push(header);
    else headers.push(HEADER_SKIP_TAG);
  }
